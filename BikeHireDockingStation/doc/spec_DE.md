...
- `address`: Die Postanschrift  - `agency_fare_url`: URL einer Webseite, die Einzelheiten zu den Tarifen enthält und auch den Online-Kauf von Fahrkarten für diese Agentur ermöglichen könnte. SameAs: Feld "agency_fare_url" aus GTFS Static Field Definition - agency.txt (https://developers.google.com/transit/gtfs/reference#agencytxt)  - `agency_name`: Das Feld agency_name enthält den vollständigen Namen des Versandunternehmens. SameAs: Feld 'agency_name' aus GTFS Static Field Definition - agency.txt (https://developers.google.com/transit/gtfs/reference#agencytxt)  - `agency_url`: Das Feld agency_url enthält die URL des Verkehrsunternehmens. SameAs: Feld 'agency_url' aus GTFS Static Field Definition - agency.txt (https://developers.google.com/transit/gtfs/reference#agencytxt)  - `alternateName`: Ein alternativer Name für diesen Artikel  - `areaServed`: Das geografische Gebiet, in dem eine Dienstleistung oder ein angebotener Artikel erbracht wird  - `availableBikeNumber`: Die Anzahl der Fahrräder, die in der Fahrradverleihstation zur Verfügung stehen und von den Nutzern ausgeliehen werden können  - `contactPoint`: Die Angaben zur Kontaktaufnahme mit dem Artikel.  - `dataProvider`: Eine Folge von Zeichen zur Identifizierung des Anbieters der harmonisierten Dateneinheit.  - `dateCreated`: Zeitstempel der Entitätserstellung. Dieser wird in der Regel von der Speicherplattform zugewiesen.  - `dateModified`: Zeitstempel der letzten Änderung der Entität. Dieser wird in der Regel von der Speicherplattform vergeben.  - `description`: Eine Beschreibung dieses Artikels  - `freeSlotNumber`: Die Anzahl der für die Rückgabe und das Abstellen von Fahrrädern verfügbaren Stellplätze. Sie muss niedriger oder gleich sein als `totalSlotNumber`.  - `id`: Eindeutiger Bezeichner der Entität  - `location`: Geojson-Referenz auf das Element. Es kann Punkt, LineString, Polygon, MultiPoint, MultiLineString oder MultiPolygon sein  - `mediaURL`: URL mit weiteren Informationen zu Bild(ern) oder Medien der Beschwerde oder des Ortes.  - `name`: Der Name dieses Artikels.  - `observationDateTime`: Letzter gemeldeter Zeitpunkt der Beobachtung.  - `openingHours`: Öffnungszeiten der Andockstation  - `outOfServiceSlotNumber`: Die Anzahl der Slots, die nicht in Ordnung sind und nicht zum Ausleihen oder Abstellen eines Fahrrads verwendet werden können. Sie muss niedriger oder gleich sein als `totalSlotNumber`.  - `owner`: Eine Liste mit einer JSON-kodierten Zeichenfolge, die auf die eindeutigen Kennungen der Eigentümer verweist  - `provider`: Anbieter von Fahrradverleihdiensten  - `seeAlso`: Liste von URLs, die auf zusätzliche Ressourcen zu dem Artikel verweisen  - `source`: Eine Folge von Zeichen, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Empfohlen wird der voll qualifizierte Domänenname des Quellanbieters oder die URL des Quellobjekts.  - `stationCode`: Die Stationsnummer oder der Stationscode der der Beobachtung entsprechenden Fahrradverleihstation.  - `stationName`: Der Name der Fahrradverleihstation, die dieser Beobachtung entspricht.  - `status`: Status der Verleihstation für Fahrräder. Enum:'working, outOfService, withIncidence, full, almostFull, empty, almostEmpty'. Oder jede andere anwendungsspezifische Angabe.  - `totalSlotNumber`: Die Gesamtzahl der von dieser Fahrradstation angebotenen Stellplätze  - `type`: NGSI-Entitätstyp. Es muss BikeHireDockingStation sein.    
Erforderliche Eigenschaften  
- `id`  - `type`    
Viele Städte bieten ein Fahrradverleihsystem für Bürger an. Diese können ein Fahrrad auf der Grundlage verschiedener Abonnementtypen ausleihen. Eine Fahrradverleihstation, an der abonnierte Nutzer ein Fahrrad ausleihen und zurückgeben können. Sie bietet Daten über die wichtigsten Funktionen und die Verfügbarkeit von Fahrrädern und freien Plätzen.  
