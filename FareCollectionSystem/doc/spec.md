...
- `address`: The mailing address  - `alternateName`: An alternative name for this item  - `areaServed`: The geographic area where a service or offered item is provided  - `cardId`: Unique ticket Id of the transaction or Id of the smart card used in the transaction.  - `currentTripCount`: The current count of trips made by the vehicle corresponding to this observation on the given day of operation.  - `dataProvider`: A sequence of characters identifying the provider of the harmonised data entity.  - `dateCreated`: Entity creation timestamp. This will usually be allocated by the storage platform.  - `dateModified`: Timestamp of the last modification of the entity. This will usually be allocated by the storage platform.  - `description`: A description of this item  - `destinationStopCategory`: Type of the destination bus stop corresponding to this observation.  - `destinationStopId`: Unique Id of the bus stop at which the passenger deboards from the bus corresponding to this observation.  - `destinationStopName`: The name of the destination bus stop corresponding to this observation.  - `direction_id`: Indicates the direction of travel of the vehicle corresponding to this observation, can be referenced from the GTFS static feed trips.txt. SameAs: 'direction_id' field from GTFS Realtime message-TripDescriptor (https://developers.google.com/transit/gtfs-realtime/reference#message-tripdescriptor)  - `entryAreaCode`: Area code of the passenger boarding stop (used by the fare collection agency). For example, whether the stop is city-bus-service stop or brts stop or other service type stop etc.  - `equipmentCompanyCode`: Company/Agency code for the transaction equipment (used by fare collection agency). For example, 103 - CBS (city bus service),102 - BRTS etc.  - `equipmentId`: Unique Id of the equipment corresponding to this observation.  - `equipmentSequenceNumber`: Sequence number for the given equipment.  - `equipmentStopId`: Stop Id (BRTS) at which the equipment corresponding to this transaction is installed.  - `equipmentType`: Type of equipment or the name of the equipment corresponding to this observation.  - `equipmentTypeCode`: Unique code indicating the type of equipment used in the transaction (used by fare collection agency).ENUM [1B, 42, 02, 08, 41] 1B - POS, 42 - HTT, 02- Mobile, 08- Fare Gate, 41- Pole Validator  - `exitAreaCode`: Area code of the passenger alighting stop (used by the fare collection agency). For example, whether the stop is city-bus-service stop or BRTS stop or other service type stop etc.  - `fareForAdult`: The fare for an adult in the journey corresponding to this observation.  - `fareForChild`: The fare for a child in the journey corresponding to this observation.  - `id`: Unique identifier of the entity  - `location`: Geojson reference to the item. It can be Point, LineString, Polygon, MultiPoint, MultiLineString or MultiPolygon  - `name`: The name of this item.  - `observationDateTime`: Last reported time of observation.  - `occupancyLevel`: Occupancy level in the public transit bus corresponding to this observation. 'Red' indicates the congestion level in the bus is HIGH, 'Yellow' indicates the congestion level in the bus is MODERATE and 'Green' indicates the congestion level in the bus is LOW.  - `originDestinationCode`: The code for the origin and destination stops corresponding to this observation.  - `originStopCategory`: Type of the origin bus stop corresponding to this observation.  - `originStopId`: Unique Id of the bus stop at which the passenger boards into the bus corresponding to this observation.  - `originStopName`: The name of the origin bus stop corresponding to this observation.  - `owner`: A List containing a JSON encoded sequence of characters referencing the unique Ids of the owner(s)  - `passengerCount`: Number of passengers travelling in the public transit bus corresponding to this observation. This count is computed based on the realtime ticketing transactions in the public transit bus.  - `route_id`: Route Id assigned to the route on which the bus/vehicle corresponding to the bus in this observation is currently plying on. SameAs: 'route_id' field from GTFS Realtime message-TripDescriptor (https://developers.google.com/transit/gtfs-realtime/reference#message-tripdescriptor)  - `seeAlso`: list of uri pointing to additional resources about the item  - `shiftOfOperation`: Shift of operation of the public transit vehicle operated by the fare collection agency/ public transit agency or the employee corresponding to this observation. Indicated as '1' when the vehicle operates in the first shift, '2' when the vehicle operates in the second shift and '3' when the vehicle operates in the third shift.  - `source`: A sequence of characters giving the original source of the entity data as a URL. Recommended to be the fully qualified domain name of the source provider, or the URL to the source object.  - `stage`: Total number of stages from the origin bus stop to the destination bus stop.  - `ticketTypeCode`: Unique ticket type code of the corresponding trip.  - `transactionDateTime`: Date-time of the transaction corresponding to this observation.  - `transactionType`: Type of the transaction corresponding to this observation. For Eg - Issue, ReIssue, Entry, Exit etc.  - `transactionTypeDescription`: Description of the transaction type. Explanation of various transactionTypeId codes used in the data.  - `transactionVehicleNum`: Code used by fare collection agency for the vehicle number corresponding to the transaction.  - `travelDistance`: The distance between the origin bus stop and the destination bus stop corresponding to this observation.  - `trip_id`: Trip Id/Trip name alloted to the bus corresponding to this observation, in consideration to the time of the day and the direction of the trip on the given routeId. SameAs: 'trip_id' field from GTFS Realtime message-TripDescriptor (https://developers.google.com/transit/gtfs-realtime/reference#message-tripdescriptor)  - `type`: NGSI entity type. It has to be FareCollectionSystem  - `vehicle_label`: User visible label, i.e., something that must be shown to the passenger to help identify the correct vehicle. SameAs: 'label' field from GTFS Realtime message-VehicleDescriptor (https://developers.google.com/transit/gtfs-realtime/reference#message-vehicledescriptor)    
...
```  
Not available the example of a FareCollectionSystem in JSON-LD format as normalized. This is compatible with NGSI-v2 when not using options and returns the context data of an individual entity.  
